...
Original file is located at
    https://colab.research.google.com/drive/19A2QIsIVW592EkRUqj44ucA1C7KspJtD
"""
...
config['param_grid'] = {
        ...
        'drop_rate': hp.uniform('drop_rate', 0.2, 0.4, 1),
...
def make_future_prediction(stock_name,  model, scaler, future_step, config):
    df = get_data(config, stock_name)
    windows_size = config['windows_size']
    input_col = config['input_col']
    output_col = config['output_col']
...
def do_main(stock_name, config):
    ...
    future_predict = make_future_prediction(stock_name, train_result['model'], train_result['scaler'] ,10, config)
